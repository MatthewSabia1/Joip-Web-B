...
export function MediaDisplay({
  ...
  // Function to handle URL processing for Reddit media
  const getProxiedUrl = (url: string | undefined): string | undefined => {
    if (!url) return undefined;
    
    // Log for debugging
    console.log(`[MediaDisplay] Processing URL: ${url}`);
    ...
    // Fix Reddit preview URLs which often have escaped characters
    processedUrl = processedUrl.replace(/&amp;/g, '&');
    
    // Remove query parameters from image URLs which can cause loading issues
    if (/\.(jpg|jpeg|png|gif|webp)(\?|$)/i.test(processedUrl)) {
      processedUrl = processedUrl.split('?')[0];
    }
    
    console.log(`[MediaDisplay] Final URL: ${processedUrl}`);
    return processedUrl;
  };
