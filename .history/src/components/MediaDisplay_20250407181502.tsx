...
import { RedditPost, TransitionEffect, Subreddit } from '@/types';
...
export function MediaDisplay({
  ...
  // Find the subreddit data for the current post
  const currentSubredditData = post ? allSubreddits.find(sub => sub.name === post.subreddit) : null;
  const subredditError = currentSubredditData?.error;
  ...
  // Display error message if the CURRENT subreddit has an error
  if (subredditError) {
    return (
      <div className="flex items-center justify-center h-full p-6">
        <Alert variant="destructive" className="max-w-md">
          <AlertTriangleIcon className="h-5 w-5" />
          <AlertTitle>Error loading r/{post?.subreddit}</AlertTitle>
