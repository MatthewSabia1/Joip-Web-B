...
*   **[Date TBD - Fill in manually]**
    *   **Feature/Fix:** Refined media URL selection logic in `useRedditPosts` hook.
    *   **Details:** Modified the image handling logic to prioritize `preview.images[0].source.url` (and its NSFW variant) from the Reddit API response over `post.url` to ensure the highest resolution image is displayed in `MediaDisplay`. This addresses the issue where low-resolution thumbnails were sometimes shown instead of the full media.
    *   **Files Changed:** `src/hooks/useRedditPosts.ts`
