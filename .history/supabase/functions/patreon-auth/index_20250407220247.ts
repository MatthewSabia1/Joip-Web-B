...
const PATREON_CLIENT_ID = "vPypbFE0lcWcdUX79jqPCs6EvRgxY5vjdOxR7-MiJvzWXu4EBtacoQC8TsRj-Q_T";
const PATREON_CLIENT_SECRET = "4G367hsywm_trNV8Ewhura1CmQl1Zv2ch-zUdOT0ekJLglcn4hEoCuYTGDbFEGjd";
const CREATOR_ACCESS_TOKEN = "IUeXq6ei-lfA7vlB716kx47z5tiZjhGSWk442C9eGnY";
const CREATOR_REFRESH_TOKEN = "Z0eVxVdjB7paNiHEViMLahkGP80DTJNg-5bCh100d7w";

// Define the correct redirect URI based on the Supabase URL
const SUPABASE_URL = Deno.env.get("SUPABASE_URL") || "https://bfserjasoryvqoiarbku.supabase.co";
const REDIRECT_URI = `${SUPABASE_URL}/functions/v1/patreon-auth/callback`;

// Define the frontend app URL for redirects
const FRONTEND_URL = "https://golden-gelato-5a2c4f.netlify.app";
  ...
  try {
    ...
    if (path === "callback") {
      ...
      if (!tokenResponse.ok) {
        ...
        console.error("[PatreonAuthFunction] Token exchange error:", error);
        return new Response(
      ...
      // Validate the state parameter to get the user ID
      const userId = state;
      if (!userId) {
        return new Response(
      ...
      console.log("[PatreonAuthFunction] Updating user profile with Patreon data for user:", userId);
      ...
      // Use the Netlify URL for the redirect
      return new Response(null, {
        status: 302,
        headers: {
          ...corsHeaders,
