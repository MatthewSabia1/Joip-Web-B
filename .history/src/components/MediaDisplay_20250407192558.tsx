...
export function MediaDisplay({
  ...
  const getProxiedUrl = (url: string) => {
    ...
    // Replace encoded HTML entities if needed
    if (url.includes('&amp;')) {
      processedUrl = url.replace(/&amp;/g, '&');
    }
    
    // Make sure URL uses HTTPS (Reddit sometimes returns HTTP)
    if (processedUrl.startsWith('http://')) {
      processedUrl = processedUrl.replace('http://', 'https://');
    }
    
    // Handle Reddit image URLs specifically
    if (processedUrl.includes('i.redd.it')) {
      // Use direct URL for Reddit images
      processedUrl = processedUrl.replace(/&.*$/, ''); // Remove query params
    }
    ...
    // In production, could use our own proxy service
    return processedUrl;
  };
          ...
          {post.isImage ? (
            <img
              ...
              onError={(e) => {
                ...
                // Create a test image to check if direct URL works
                const testImg = new Image();
                testImg.onload = () => {
                  imgElement.src = directUrl;
                };
                testImg.onerror = () => {
                  // If direct URL fails, try thumbnail as fallback
                  if (post.thumbnailUrl && post.thumbnailUrl !== "self" && post.thumbnailUrl !== "default") {
                    console.log("Trying thumbnail as fallback:", post.thumbnailUrl);
            ...
            <video
              ...
              onError={(e) => {
                ...
                try {
                  const directUrl = post.videoUrl;
                  const videoElement = e.target as HTMLVideoElement;
                  videoElement.src = directUrl;
                  
                  // Add event listener for the second attempt
                  videoElement.onerror = () => {
                    // If that fails too, try thumbnail
                    if (post.thumbnailUrl && post.thumbnailUrl !== "self" && post.thumbnailUrl !== "default") {
                      console.log("Video failed, displaying thumbnail instead:", post.thumbnailUrl);
                  ...
                  console.error("Error during video fallback:", error);
                  setIsImageLoading(false);
                }
              }}
            />
            ...
            <div className="flex items-center justify-center h-full">
              {post.thumbnailUrl && post.thumbnailUrl !== "self" && post.thumbnailUrl !== "default" ? (
                <img
                  ...
                  onLoad={handleImageLoad}
                  onError={() => {
                    console.error("Failed to load thumbnail:", post.thumbnailUrl);
                    // Try direct URL without proxy
                    const imgElement = document.querySelector(`img[alt="${post.title}"]`) as HTMLImageElement;
                    if (imgElement) {
                      imgElement.src = post.thumbnailUrl;
                    }
                    setIsImageLoading(false);
