...
export function MediaDisplay({
  ...
  const getProxiedUrl = (url: string | undefined): string | undefined => {
    ...
    // IMPORTANT: Try direct URLs first, many Reddit URLs should work directly
    if (processedUrl.includes('i.redd.it') || 
        processedUrl.includes('preview.redd.it') ||
        processedUrl.includes('i.imgur.com')) {
      console.log(`[Media] Using direct URL for known domain: ${processedUrl}`);
      return processedUrl;
    }
    
    // Only proxy v.redd.it videos and domains known to need CORS handling
    const needsProxy = processedUrl.includes('v.redd.it') || 
                      processedUrl.includes('redgifs.com') ||
                      processedUrl.includes('gfycat.com');
                      
    if (needsProxy) {
      // For local development, use a CORS proxy service
      if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
        console.log(`[Proxy] Using CORS proxy for: ${processedUrl}`);
        // Try a more reliable proxy
        return `https://corsproxy.io/?${encodeURIComponent(processedUrl)}`;
      }
    }
    
    // In production or for other URLs, return the processed URL directly
    return processedUrl;
  };
        ...
        <div className={cn("h-full w-full flex items-center justify-center", getTransitionClasses())}>
          ...
          {post.isVideo && post.videoUrl ? (
            <video
              ...
              onError={(e) => {
                console.error(`[Video Load Error] Failed for post ${post.id}: ${getProxiedUrl(post.videoUrl)}`, e);
                // Simple error handling: Log error, perhaps show a placeholder? 
                // For now, just ensuring the loading state is false.
                setIsImageLoading(false); 
                // We could potentially try rendering the `post.url` (image preview) here as a fallback
          ...
          : post.isImage && post.url ? (
            <img
              ...
              onError={(e) => {
                console.error(`[Image Load Error] Failed for post ${post.id}: ${getProxiedUrl(post.url)}`, e);
                // Simple error handling: Log error. 
                setIsImageLoading(false); 
                // Could try thumbnail here if needed, but keeping it simple.
              }}
            />
          ) 
          ...
          : post.thumbnailUrl ? (
             <img
               ...
               onLoad={handleImageLoad}
               onError={(e) => {
                 console.error(`[Thumbnail Load Error] Failed for post ${post.id}: ${getProxiedUrl(post.thumbnailUrl)}`, e);
                 setIsImageLoading(false); // Final fallback failed
               }}
             />
          ) 
          /* If no media at all */
          : (
            <div className="flex items-center justify-center h-full">
               <Alert variant="destructive" className="w-auto">
                 ...
                 <AlertDescription>Media not available for this post.</AlertDescription>
               </Alert>
            </div>
          )}
        </div>
