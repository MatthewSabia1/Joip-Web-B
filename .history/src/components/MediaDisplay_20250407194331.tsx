...
export function MediaDisplay({
  ...
  useEffect(() => {
    console.log('[MediaDisplay] Reddit auth state:', authState.isAuthenticated ? 'Authenticated' : 'Not authenticated');
  }, [authState]);
  ...
  useEffect(() => {
    if (post) {
      ...
      setMediaUrl(null); // Reset media URL when post changes
    }
  }, [post?.id]);

  // Fetch media URL when post changes
  useEffect(() => {
    const fetchMediaUrl = async () => {
      if (!post || !authState.isAuthenticated) return;
      
      try {
        // Direct approach - just use Reddit URLs directly
        console.log(`[MediaDisplay] Using direct URL: ${post.url}`);
        setMediaUrl(post.url || post.thumbnailUrl || null);
      } catch (error) {
        console.error(`[MediaDisplay] Error fetching media:`, error);
        setMediaUrl(post.thumbnailUrl || null); // Fallback to thumbnail
      }
    };
  ...
  // Using direct URLs for the media
  return (
        ...
        <div className={cn("h-full w-full flex items-center justify-center", getTransitionClasses())}>
          {post.isVideo && post.videoUrl ? (
            <video
              ...
              onError={(e) => {
                console.error(`[Video Load Error] Failed for post ${post.id}:`, e);
                setIsImageLoading(false);
              }}
            />
          ) : (
            <img
              ...
              onError={(e) => {
                console.error(`[Image Load Error] Failed for post ${post.id}:`, e);
                // Try falling back to thumbnail if main image fails
                if (post.url !== post.thumbnailUrl && post.thumbnailUrl) {
                  console.log(`[Image] Falling back to thumbnail: ${post.thumbnailUrl}`);
                  e.currentTarget.src = post.thumbnailUrl;
                } else {
                  setIsImageLoading(false);
                }
              }}
