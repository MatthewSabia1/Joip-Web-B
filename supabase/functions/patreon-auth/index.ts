...
// Creator tokens are used for webhook verification and creator-specific actions
    ...
    if (!PATREON_CLIENT_ID || !PATREON_CLIENT_SECRET || !SUPABASE_URL) {
      ...
      return new Response(
        JSON.stringify({ 
          ...
          detail: `Missing: ${!PATREON_CLIENT_ID ? 'PATREON_CLIENT_ID ' : ''}${!PATREON_CLIENT_SECRET ? 'PATREON_CLIENT_SECRET ' : ''}${!SUPABASE_URL ? 'SUPABASE_URL' : ''}`
        }),
        {
          status: 500,
    ...
    // Webhook endpoint for Patreon events
      ...
      if (PATREON_WEBHOOK_SECRET) {
        try {
          ...
          // Reconstruct the raw request body as a string
          const requestBody = JSON.stringify(webhookData);
          
          // Create HMAC-MD5 using the webhook secret
          const encoder = new TextEncoder();
          const key = await crypto.subtle.importKey(
            "raw", 
            encoder.encode(PATREON_WEBHOOK_SECRET),
            { name: "HMAC", hash: "MD5" },
            false,
            ["sign"]
          );
