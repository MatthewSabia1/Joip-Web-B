        ...
        <div className={cn("h-full w-full flex items-center justify-center", getTransitionClasses())}>
          ...
          {post.isVideo && post.videoUrl ? (
            <video
              ...
              onError={(e) => {
                console.error(`[Video Error] Failed to load: ${post.videoUrl}`, e);
                setIsImageLoading(false);
              }}
            />
          ) : (
            /* Handle Images - Direct approach without proxies */
            <img
              key={post.id + '-image'}
              ...
              onError={(e) => {
                console.error(`[Image Error] Failed to load image: ${post.url}`, e);
                // If main image URL fails and we have a thumbnail, try that instead
                if (post.url !== post.thumbnailUrl && post.thumbnailUrl) {
                  console.log(`[Image Recovery] Trying thumbnail instead: ${post.thumbnailUrl}`);
