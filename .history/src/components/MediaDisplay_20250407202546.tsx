...
import { AlertTriangleIcon, RefreshCwIcon as RefreshIcon, LogIn } from 'lucide-react';
...
export function MediaDisplay({
  ...
  // Add a timeout for the auth loading state
  useEffect(() => {
    let timeoutId: number | undefined;
    
    if (isAuthLoading) {
      // If still loading after 15 seconds, show a timeout message
          ...
          {authLoadingTimeout && (
            <div className="max-w-md mx-auto">
              <p className="text-sm text-muted-foreground mt-2">
                Reddit API might be experiencing issues. You can:
              </p>
  ...
  const getTransformedUrl = (url: string | undefined) => {
    ...
    // -- REMOVED: Conversion from preview.redd.it to i.redd.it --
    // This might have been causing CORS issues if preview.redd.it had better headers.
    // Let's try using preview.redd.it directly if provided.
    /*
    ...
    */
    ...
    // Remove query parameters from image URLs (still useful)
    if (/\.(jpg|jpeg|png|gif|webp)/i.test(url) && url.includes('?')) {
      return url.split('?')[0];
    }
    
    // Return the URL as is if no transformation applies
    return url;
  };
